...
function isStorageGateOpen(): boolean {
  const dedicated = process.env['STORAGE_SECRET']?.trim();
  if (dedicated) return true;
  // Repli : NEXTAUTH_SECRET est déjà requis en prod ; évite les échecs si .env.local omet STORAGE_SECRET
